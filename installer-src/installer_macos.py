#!/usr/bin/env python3
"""Claisum macOS Installer — v1.0.0.1"""
...
INJECT  = (
    "// [Claisum] Do NOT remove — uninstall via Claisum_macOS_Setup --remove\n"
...
# ── Discord path detection (macOS) ─────────────────────────────────────────
def find_discord_indices() -> list[str]:
    lib = os.path.expanduser("~/Library/Application Support")
    bases = [
        os.path.join(lib, "discord"),
        os.path.join(lib, "discordptb"),
        os.path.join(lib, "discordcanary"),
        os.path.join(lib, "discorddev"),
        # Flatpak / alternative installs
...
def kill_discord() -> None:
    subprocess.run(["osascript", "-e",
        'tell application "Discord" to quit'], capture_output=True)
    time.sleep(0.5)
...
def banner() -> None:
    print(f"""
{BLD('══════════════════════════════════════')}
  {CYN('⚡ Claisum Installer')}  {DIM(f'v{VERSION}')}  {DIM('macOS')}
...
def run_install() -> int:
    ...
    if not indices:
        ...
        warn("Make sure Discord has been launched at least once after installing.")
        return 1
...
""")
    ...
    if ans in ("", "y"):
        if relaunch_discord():
            ...
            warn("Could not find Discord.app — restart manually.")
    return 0
