// Claisum — Renderer Preload v9
// Injected via resources/app/index.js (BrowserWindow patch).
// Preload context: full DOM + web APIs available regardless of contextIsolation.

// ── 0. Chain Discord's original preload (if any) ─────────────────────────────
(function () {
  try {
    var orig = (process && process.argv || []).filter(function (a) {
      return String(a).indexOf('--claisum-orig-preload=') === 0;
    });
    if (orig.length) {
      var p = orig[0].slice('--claisum-orig-preload='.length);
      if (p) require(p);
    }
  } catch (e) { console.error('[Claisum] orig preload chain failed:', e); }
})();

// ── 1. Diagnostic desktop log ────────────────────────────────────────────────
(function () {
  try {
    var fs   = require('fs');
    var os   = require('os');
    ...
    fs.writeFileSync(
      path.join(os.homedir(), 'Desktop', 'claisum_RENDERER.txt'),
      '[Claisum] Preload ran: ' + new Date().toISOString() + '\n' +
        ...
        xhr.onreadystatechange=function(){
          ...
          try{
            ...
            if(tag&&tag!==VERSION){
              ...
              fab.appendChild(b); fab.title='Claisum v'+tag+' verfügbar!';
            }
          }catch(e){}
        };
