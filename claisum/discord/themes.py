...
from claisum.discord.paths import get_discord_css_path
...
def list_themes() -> None:
    ...
    for key, theme in BUILTIN_THEMES.items():
        table.add_row(key, theme["description"], theme["author"], "built-in")
...
def apply_theme(theme_name: str) -> bool:
    ...
    css_path = get_discord_css_path()
    if not css_path:
        console.print("[red]Could not find Discord installation.[/red]")
        console.print("[dim]Make sure Discord is installed and try again.[/dim]")
        return False
    ...
    if css is None:
        console.print(f"[red]Theme '[bold]{theme_name}[/bold]' not found.[/red]")
    ...
    console.print(f"[green]Theme '[bold]{theme_name}[/bold]' applied successfully.[/green]")
...
def remove_theme() -> bool:
    ...
    console.print("[green]Theme removed. Restart Discord to see the changes.[/green]")
    return True
